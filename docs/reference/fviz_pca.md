...
with minimal loss of information. fviz_pca() provides ggplot2-based
...
function
[`fviz()`](https://erdeyl.github.io/factoextraplus/reference/fviz.md),
which is also a wrapper around the function
[`ggscatter()`](https://rdrr.io/pkg/ggpubrplus/man/ggscatter.html) \[in
ggpubrplus\]. Therefore, further arguments, to be passed to the function
[`fviz()`](https://erdeyl.github.io/factoextraplus/reference/fviz.md)
and [`ggscatter()`](https://rdrr.io/pkg/ggpubrplus/man/ggscatter.html),
can be specified in `fviz_pca_ind()` and `fviz_pca_var()`.
...
- ...:
  ...
  - in fviz_pca_ind() and fviz_pca_var(): Additional arguments are
    passed to the functions fviz() and ggpubrplus::ggpar().

  - in fviz_pca_biplot() and fviz_pca(): Additional arguments are passed
    to fviz_pca_ind() and fviz_pca_var().
...
- geom.ind, geom.var:
  ...
  geom.ind = c("point", "text), geom.var = c("arrow", "text").
...
- label:
  ...
  individuals. "ind.sup" is for supplementary individuals. "quali" is
  for supplementary qualitative variables. "var" is for active
  variables. "quanti.sup" is for quantitative supplementary variables.
...
- biplot.type:
  ...
  - "form": Form biplot (Gabriel, 1971). Distances between individuals
    approximate Euclidean distances. Use when focus is on individual
    relationships.

  - "covariance": Covariance biplot (Gabriel, 1971). Angles between
    variable vectors approximate correlations, lengths approximate
    standard deviations. Use when focus is on variable relationships.

  Note: "form" and "covariance" scaling requires prcomp or princomp
  objects.
